...
The current **release line** described here is **`v1.0.1`** (Git tag, package **`1.0.1`**, & `SCANNER_VERSION` in code). Keep those aligned when you cut **`v1.0.2`** or later.
...
1. **Checks out** a **pinned release tag** of the repository (default **`v1.0.1`**) so the scan runs against a **known, immutable** tree.
...
4. **Use workflow from** — Select **`v1.0.1`** (the release tag). That loads the **workflow YAML** from the same snapshot as the scanner checkout, so **documentation, workflow inputs, & code stay aligned**. GitHub may **default** this dropdown to your **default branch** (often named `main`); change it to **`v1.0.1`** unless you are deliberately testing an unreleased workflow change.
5. **Pinned ReckLock Discover release tag to scan** — leave **`v1.0.1`** (or pick the only option). This controls **`actions/checkout`** & therefore **which snapshot of the codebase** gets scanned. **Match this to the same tag you chose in step 4** for normal runs.
...
```bash
gh workflow run recklock-discover.yml --ref v1.0.1 -f recklock_release=v1.0.1
```
...
| Input | Meaning |
| --- | --- |
| **`recklock_release`** | Git ref (tag) to check out before scanning. Today the only choice is **`v1.0.1`**, matching the [release tag](https://github.com/robbiej24/ReckLockDiscover/releases/tag/v1.0.1) on the OSS repo once published. |
...
**Two GitHub dropdowns:** **Use workflow from** picks the **workflow file** revision; **`recklock_release`** picks the **checkout** for the scan. If you mix—for example **Use workflow from** = default branch & **`recklock_release`** = **`v1.0.1`**—you may run **newer YAML** against an **older** tree, or the reverse. For **predictable, reviewable runs**, set **both** to **`v1.0.1`**.
...
**Upgrades:** when you ship **`v1.0.2`**, add it to the workflow **`options`** list, bump **`pyproject.toml`**, **`recklock_scanner/constants.py`**, & this doc together, then tag & mirror.
